...
static uint8_t W25Q64_SwapByte(uint8_t txByte)
{
    ...
    for (uint8_t i = 0; i < 8; i++)
    {
        ...
        /* CLK 上升沿 — Flash 输出 MISO, 采样 */
        W25Q64_CLK_HIGH();
        delay_us(SPI_HALF_CYCLE_US);

        rxByte <<= 1;
        rxByte  |= W25Q64_MISO_READ();
...
uint32_t W25Q64_ReadID(void)
{
    ...
    id  = (uint32_t)W25Q64_SwapByte(0xFF) << 16;
    id |= (uint32_t)W25Q64_SwapByte(0xFF) << 8;
    id |= (uint32_t)W25Q64_SwapByte(0xFF);
    W25Q64_CS_HIGH();
